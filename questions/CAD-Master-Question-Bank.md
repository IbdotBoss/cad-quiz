...
| Q106 | B | web | The `.*` wildcard covers all rows and all fields. Contrast `table.None`, which is the table-level (record) rule. |
...
| Q126 | A & B | web | ACLs control create, read, write and delete (plus execute, report_on and similar). "Compile" and "Index" are not database operations ACLs govern. |
...
| Q132 | A | web | `previous` is unavailable in a Script Action — the event is decoupled from the original database transaction. |
...
| Q135 | B | web | The **watermark** appended to notification emails. |
...
| Q143 | A | web | Query. Async rules run after the database transaction, triggered by insert/update/delete — a read does not schedule async work. |
...
| Q147 | A | web | URL (from Arguments), pointing at the properties page. |
...
| Q152 | C | web | `gs.getUserSysID()` returns the sys_id; `gs.getUserID()` is the deprecated/alternate form and `g_form` is client-side. ⚠️ Mined sources garbled the options here — learn the pair: `getUserSysID()` = sys_id, `getUserName()` = user name. |
